...
import re
pattern = r"int wolfSSL_quic_aead_encrypt\(uint8_t\* dest, WOLFSSL_EVP_CIPHER_CTX\* ctx,\s*const uint8_t\* plain, size_t plainlen,\s*const uint8_t\* iv, const uint8_t\* aad,\s*size_t aadlen\)\s*\{\s*int len;\s*/\*.*?return WOLFSSL_SUCCESS;\s*\}"
