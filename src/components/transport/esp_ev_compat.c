...
#define EV_COMPAT_EVENT_LOOP_STACK_SIZE 8192
#define EV_COMPAT_IO_TASK_STACK_SIZE 4096
...
static esp_err_t ev_loop_init(ev_loop *loop) {
    ...
    esp_event_loop_args_t loop_args = {
        ...
        .task_priority = 5,
        .task_stack_size = EV_COMPAT_EVENT_LOOP_STACK_SIZE,
...
esp_err_t ev_default_loop_init(void) {
    return ev_loop_init(EV_DEFAULT);
}
...
// Start an IO watcher
esp_err_t ev_io_start(ev_loop *loop, ev_io *watcher) {
    ...
    if (idx >= 0) {
        ...
        if (loop->io_count == 0) {
            ...
            if (!loop->running) {
                loop->running = true;
                BaseType_t task_created = xTaskCreate(io_monitor_task,
                    "io_monitor",
                    EV_COMPAT_IO_TASK_STACK_SIZE,
                    loop,
                    5,
                    &loop->io_task_handle);
                if (task_created != pdPASS) {
                    ESP_LOGE(TAG, "Failed to create IO monitor task");
                    loop->running = false;
                    loop->io_watchers[idx] = NULL;
                    watcher->active = 0;
                    xSemaphoreGive(loop->io_mutex);
