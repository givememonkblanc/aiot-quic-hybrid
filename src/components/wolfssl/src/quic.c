                               ...
                               size_t aadlen)
{
    ...
    inbuf = (uint8_t*)XMALLOC(plainlen, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    outbuf = (uint8_t*)XMALLOC(plainlen + (size_t)ctx->authTagSz, NULL,
                               DYNAMIC_TYPE_TMP_BUFFER);
    ...
    if (wolfSSL_EVP_CipherInit(ctx, NULL, NULL, iv, 1) != WOLFSSL_SUCCESS
        ...
        || wolfSSL_EVP_CipherUpdate(
                ctx, outbuf, &len, inbuf, (int)plainlen) != WOLFSSL_SUCCESS
        || wolfSSL_EVP_CipherFinal(ctx, outbuf + len, &len) != WOLFSSL_SUCCESS
        || wolfSSL_EVP_CIPHER_CTX_ctrl(
                ctx, WOLFSSL_EVP_CTRL_AEAD_GET_TAG, ctx->authTagSz, outbuf + plainlen)
            != WOLFSSL_SUCCESS) {
        XFREE(inbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        XFREE(outbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        return WOLFSSL_FAILURE;
    ...
    XMEMCPY(dest, outbuf, plainlen + (size_t)ctx->authTagSz);
    XFREE(inbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    XFREE(outbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    return WOLFSSL_SUCCESS;
}
                                ...
                                size_t aadlen)
{
    ...
    inbuf = (uint8_t*)XMALLOC(enclen, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    outbuf = (uint8_t*)XMALLOC(enclen, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    if (inbuf == NULL || outbuf == NULL) {
        if (inbuf != NULL) {
            XFREE(inbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        }
    ...
    if (wolfSSL_EVP_CipherInit(ctx, NULL, NULL, iv, 0) != WOLFSSL_SUCCESS
        ...
        || wolfSSL_EVP_CipherUpdate(ctx, NULL, &len, aad, (int)aadlen)
            != WOLFSSL_SUCCESS
        || wolfSSL_EVP_CipherUpdate(ctx, outbuf, &len, inbuf, (int)enclen)
            != WOLFSSL_SUCCESS
        || wolfSSL_EVP_CipherFinal(ctx, outbuf, &len) != WOLFSSL_SUCCESS) {
        XFREE(inbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        XFREE(outbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
        return WOLFSSL_FAILURE;
    }

    XMEMCPY(dest, outbuf, enclen);
    XFREE(inbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    XFREE(outbuf, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    return WOLFSSL_SUCCESS;
}
